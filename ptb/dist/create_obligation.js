...
import { SCALLOP_PACKAGE_ID, SCALLOP_VERSION_ID, SCALLOP_MARKET_ID, SUI_TYPE, RPC_URL, } from "./types.js";
// 0.1 SUI as collateral — enough to open an obligation without exhausting gas
const COLLATERAL_AMOUNT = 100000000n;
function keypair() {
    const key = process.env.USER_PRIVATE_KEY;
    if (!key)
        throw new Error("USER_PRIVATE_KEY not set");
...
async function main() {
    ...
    const obligationChange = result.objectChanges?.find((c) => c.type === "created" && c.objectType?.includes("::obligation::Obligation"));
    const obligationKeyChange = result.objectChanges?.find((c) => c.type === "created" && c.objectType?.includes("::obligation::ObligationKey"));
