...
async function main() {
    ...
    if (!newAgentAddress)
        throw new Error("NEW_AGENT_ADDRESS not set");
    const policyId = process.env.RISK_POLICY_ID;
    if (!policyId)
        throw new Error("RISK_POLICY_ID not set");
    ...
    const delegationChange = result.objectChanges?.find((c) => c.type === "created" && c.objectType?.includes("guardian_cap::GuardianDelegation"));
