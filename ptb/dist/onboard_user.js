...
import { PACKAGE_ID, SHIELD_REGISTRY_ID, SUI_CLOCK_ID, RPC_URL, } from "./types.js";
function userKeypair() {
    const key = process.env.USER_PRIVATE_KEY;
    if (!key)
        throw new Error("USER_PRIVATE_KEY env var not set");
...
export async function onboardUser(opts) {
    ...
    const positionChange = r1.objectChanges?.find((c) => c.type === "created" && c.objectType?.includes("shield_registry::ProtectedPosition"));
    const guardianCapChange = r1.objectChanges?.find((c) => c.type === "created" && c.objectType?.includes("guardian_cap::GuardianCap"));
    ...
    const snapshotChange = r2.objectChanges?.find((c) => c.type === "created" && c.objectType?.includes("shield_oracle::RiskSnapshot"));
    const vaultChange = r2.objectChanges?.find((c) => c.type === "created" && c.objectType?.includes("shield_vault::ShieldVault"));
